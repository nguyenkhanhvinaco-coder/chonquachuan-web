...
export async function generateMetadata({ searchParams }: { searchParams: SP }): Promise<Metadata> {
  const { tu, den } = readParams(searchParams);
  const qs = buildQuery(searchParams);
  const imageUrl = `${SITE_URL}/thiep-trung-thu/xem/anh?${qs.toString()}`;
  const title = `${tu} gửi lời chúc Trung Thu đến ${den} 🏮`;
  const description = "Xem thiệp và gửi lời chúc Trung Thu của riêng bạn — miễn phí, từ Chọn Quà Chuẩn.";
...
export default function XemThiepPage({ searchParams }: { searchParams: SP }) {
  const { tu, den, loiNhan } = readParams(searchParams);
  const qs = buildQuery(searchParams);
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex-1 flex flex-col items-center gap-8 px-6 py-12 bg-surface-2">
        <CardScene tu={tu} den={den} loiNhan={loiNhan} />
        <CardActions
          shareUrl={shareUrl}
          imageUrl={imageUrl}
          den={den}
          zaloOaUrl={process.env.NEXT_PUBLIC_ZALO_OA_URL}
        />
      </div>
    </div>
  );
